...
AWG_SUBNET=$(awk '/^Address/{split($3,a,"/"); print a[1]}' /opt/amnezia/awg/wg0.conf 2>/dev/null || echo "10.8.0.0")
AWG_CIDR=$(awk '/^Address/{split($3,a,"/"); print a[2]}' /opt/amnezia/awg/wg0.conf 2>/dev/null || echo "24")
...
iptables -A FORWARD -i wg0 -o eth0 -s "${AWG_SUBNET}/${AWG_CIDR}" -j ACCEPT
iptables -A FORWARD -i wg0 -o eth1 -s "${AWG_SUBNET}/${AWG_CIDR}" -j ACCEPT
